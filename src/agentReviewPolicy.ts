...
export const AGENT_CONTEXT_BOOTSTRAP_GUIDELINES = [
  ...
  'Before the first AI review pass, start from the current Markdown target and discover only the repo context needed for the requested review or edit.',
  'Human onboarding should start from a short bootstrap prompt, not from a status panel or a blank context file.',
  'The bootstrap prompt should work with any AI agent: discover relevant repo context, ask only for missing context, use AI Markdown Review Loop tools when available, and preserve colocated sidecar review metadata during Markdown edits.',
  'If a durable context brief is missing or stale, the AI can draft or refresh one when the human asks, but brief creation is not the only outcome of bootstrap.',
